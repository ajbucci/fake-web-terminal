...
function setCaretPosition(inputCmd, promptDiv) {
  caret.style.left =
    inputCmd.selectionStart * parseFloat(getComputedStyle(caret).width) + // need to compute the decimal # of pixels to multiply
...
function setCaretContent(inputCmd) {
  if (
    (inputCmd.value.length == 0) |
    (inputCmd.selectionStart > inputCmd.value.length)
  ) {
    caret.textContent = '';
  } else {
